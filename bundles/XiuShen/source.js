var _Sources = (() => {
  ...
  // src/XiuShen/XiuShen.ts
  var XiuShen_exports = {};
  __export(XiuShen_exports, {
    XiuShen: () => XiuShen,
    XiuShenInfo: () => XiuShenInfo
  });
  ...
  // src/XiuShen/parser.ts
  var BASE_URL = "https://m.xsnvshen.com";
  function parseAlbumList(html3) {
    const $2 = load(html3);
    const items = [];
    $2("ul.pic-list li, .album-list li, li.pic-item").each(
      (_, el) => {
        const $el = $2(el);
  ...
  function parseAlbumDetail(html3) {
    const $2 = load(html3);
    const title = $2("h1, .album-title, title").first().text().trim();
    const cover = $2("img").first().attr("src") || "";
    const author = $2(".model-name, .author, .girl-name").first().text().trim() || "Unknown";
    const tags = [];
    $2(".tags a, .tag-list a, .label a").each(
      (_, el) => {
        tags.push($2(el).text().trim());
      }
    );
    const images = [];
    $2("ul.pic-list img, .swiper-slide img, .photo-list img, #piclist img").each(
      (_, el) => {
  ...
  // src/XiuShen/XiuShen.ts
  var BASE_URL2 = "https://m.xsnvshen.com";
  var XiuShenInfo = {
    version: "1.0.0",
    name: "XiuShen",
    icon: "icon.png",
  ...
  var XiuShen = class extends import_types2.Source {
    ...
    async getSearchResults(query, metadata) {
      ...
      const url = keyword ? `${BASE_URL2}/search/?k=${encodeURIComponent(keyword)}&p=${page}` : buildListUrl(page);
